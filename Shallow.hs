...
plot width height region =
    -- Se foloseste intercalate pentru a combina mai multe liste folosind un element de legatura ("\n" in acest caz)
    intercalate "\n" $ 
    -- Se foloseste list comprehension pentru a genera sirul de caractere
...
combineTransformations = foldr1 (.) . reverse
...
bfs start expand = bfsHelper [(start, 0)] S.empty 
    where
        -- Daca coada este goala, inseamna ca am terminat cautarea si returnam o lista goala.
        bfsHelper [] _ = [] 
        -- Altfel, luam prima stare din coada si restul coadei.
        bfsHelper ((state, dist):rest) visited = 
            -- Daca starea curenta a fost deja vizitata,
            if S.member state visited 
                -- atunci functia bfsHelper este apelata recursiv cu restul coadei si setul de stari vizitate.
                then bfsHelper rest visited 
                -- Altfel, starea curenta si distanta ei sunt adaugate la rezultat,
                -- iar functia bfsHelper este apelata recursiv cu coada si setul de stari vizitate actualizate.
                else (state, dist) : bfsHelper updatedQueue updatedVisited 
                where
                    -- Starile noi sunt generate prin extinderea starii curente.
                    -- Doar starile care nu au fost inca vizitate sunt adaugate la starile noi.
                    newStates = foldr (\s acc -> if S.member s visited then acc else (s, dist + 1) : acc) [] (expand state)
                    -- Coada este actualizata prin adaugarea noilor stari la sfarsit.
                    updatedQueue = rest ++ newStates 
                    -- Setul de stari vizitate este actualizat prin adaugarea starii curente.
                    updatedVisited = S.insert state visited 
...
regionAvoidingBfs start region = bfs start expand
    where
        -- Functia de expandare genereaza toate punctele adiacente unui punct dat (x, y), 
        -- dar filtreaza acele puncte care se afla in regiunea pe care dorim sa o ocolim.
