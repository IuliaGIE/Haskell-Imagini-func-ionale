...
instance Show TransformationAST where
  -- Se defineste funcția show pentru o transformare
  show (T transformation) = case transformation of
    -- Daca transformarea este o translatie, se returneaza un sir de caractere in formatul "+(tx,ty)"
    Translation tx ty -> "+(" ++ show tx ++ "," ++ show ty ++ ")"
    -- Daca transformarea este o scalare, se returneaza un sir de caractere în formatul "*<f>"
    Scaling f -> "*<" ++ show f ++ ">"
    -- Daca transformarea este o combinatie de transformari, 
    -- se returneaza o lista de siruri de caractere reprezentand fiecare transformare
    Combine transformations -> "[" ++ (concat [show t ++ "," | t <- init transformations]) ++ show (last transformations) ++ "]"
...
instance Show RegionAST where
    -- Definim cum sa afisam un RegionAST
    show r = trim $ unlines $ showIndent r 0
      where
        {- -- Definim cum sa afisam un Region cu indentare
        showIndentt level (R region) = case region of
            -- Daca regiunea este definita prin puncte, afisam punctele
            FromPoints points -> [indent ++ "FromPoints " ++ show points]
            -- Daca regiunea este un dreptunghi, afisam latimea si inaltimea
            Rectangle w h -> [indent ++ "Rectangle " ++ show w ++ " " ++ show h]
            -- Daca regiunea este un cerc, afisam raza
            Circle r -> [indent ++ "Circle " ++ show r]
            -- Daca regiunea este o transformare, afisam transformarea si regiunea transformata
            Transform t r -> [indent ++ show t] ++ showIndent (level + 1) r
            -- Daca regiunea este complementul unei alte regiuni, afisam regiunea originala
            Complement r -> [indent ++ "~"] ++ showIndent (level + 1) r
            -- Daca regiunea este uniunea a doua regiuni, afisam ambele regiuni
            Union r1 r2 -> [indent ++ "+"] ++ showIndent (level + 1) r1 ++ showIndent (level + 1) r2
            -- Daca regiunea este intersectia a doua regiuni, afisam ambele regiuni
            Intersection r1 r2 -> [indent ++ "*"] ++ showIndent (level + 1) r1 ++ showIndent (level + 1) r2
          where
            -- Definim indentarea ca fiind un numar de spatii proportional cu nivelul de adancime
            indent = replicate (2 * level) ' '
        -- Eliminam liniile goale de la inceputul si sfarsitul stringului
        trim = reverse . dropWhile (=='\n') . reverse . dropWhile (=='\n')
-}

        showIndent (R region) level = case region of
            -- Daca regiunea este definita prin puncte, afisam punctele
            FromPoints points -> [indent ++ "FromPoints " ++ show points]
            -- Daca regiunea este un dreptunghi, afisam latimea si inaltimea
            Rectangle w h -> [indent ++ "Rectangle " ++ show w ++ " " ++ show h]
            ...
            Circle r -> [indent ++ "Circle " ++ show r]
            -- Daca regiunea este o transformare, afisam transformarea si regiunea transformata
            Transform t r -> [indent ++ show t] ++ showIndent r (level + 1)
            -- Daca regiunea este complementul unei alte regiuni, afisam regiunea originala
            Complement r -> [indent ++ "~"] ++ showIndent r (level + 1)
            -- Daca regiunea este uniunea a doua regiuni, afisam ambele regiuni
            Union r1 r2 -> [indent ++ "+"] ++ showIndent r1 (level + 1) ++ showIndent r2 (level + 1)
            -- Daca regiunea este intersectia a doua regiuni, afisam ambele regiuni
            Intersection r1 r2 -> [indent ++ "*"] ++ showIndent r1 (level + 1) ++ showIndent r2 (level + 1)
          where
            -- Definim indentarea ca fiind un numar de spatii proportional cu nivelul de adancime
            indent = replicate (2 * level) ' '
        -- Eliminam liniile goale de la inceputul si sfarsitul stringului
...
instance Num RegionAST where
  --Transforma un intreg într-o regiune cu un singur punct
    fromInteger n = fromPoints [(fromInteger n, fromInteger n)]
    -- Complementul unei regiuni
    negate = complement
    -- Reuniunea a doua regiuni
    (+) = union
    -- Intersectia a doua regiuni
    (*) = intersection
...
instance Functor TransformationShape where
    ...
    -- Aplicam functia f pe transformare
    fmap f transformation = case transformation of
      -- Daca transformarea este de tipul Translation, o returnam nemodificata
      Translation x y -> Translation x y
      -- Daca transformarea este de tipul Scaling, o returnam nemodificata
      Scaling s -> Scaling s
      -- Daca transformarea este de tipul Combine, aplicam functia f pe fiecare element din lista
      Combine xs -> Combine (f <$> xs)
...
instance Functor RegionShape where
    ...
    -- Aplicam functia f pe regiune
    fmap f region = case region of
      -- Daca regiunea este de tipul FromPoints, o returnam nemodificata
      FromPoints points -> FromPoints points
      -- Daca regiunea este de tipul Rectangle, o returnam nemodificata
      Rectangle width height -> Rectangle width height
      -- Daca regiunea este de tipul Circle, o returnam nemodificata
      Circle radius -> Circle radius
      -- Daca regiunea este de tipul Complement, aplicam functia f pe interior
      Complement inner -> Complement (f inner)
      -- Daca regiunea este de tipul Union, aplicam functia f pe ambele parti
      Union left right -> Union (f left) (f right)
      -- Daca regiunea este de tipul Intersection, aplicam functia f pe ambele parti
      Intersection left right -> Intersection (f left) (f right)
      -- Daca regiunea este de tipul Transform, aplicam functia f pe interior
      Transform transformation inner -> Transform transformation (f inner)
...
toTransformation = foldTransformationAST combiner
  where
    -- 'combiner' este o functie care defineste cum sa combinam transformarile.
    combiner :: TransformationCombiner Transformation
    -- Daca transformarea este o translatie, aplicam functia 'S.translation'.
    combiner (Translation tx ty) = S.translation tx ty
    -- Daca transformarea este o scalare, aplicam functia 'S.scaling'.
    combiner (Scaling s) = S.scaling s
    -- Daca transformarea este o combinatie de transformari, le combinam folosind functia 'foldl'.
...
basicTransformationCount = foldTransformationAST combiner
  where
    -- 'combiner' este o functie care defineste cum sa numaram transformarile.
    combiner :: TransformationCombiner Int
    -- Daca transformarea este o translatie, numaram ca o transformare.
    combiner (Translation _ _) = 1
    -- Daca transformarea este o scalare, numaram ca o transformare.
    combiner (Scaling _) = 1
    -- Daca transformarea este o combinatie de transformari, numaram numarul total de transformari.
    combiner (Combine transformationCounts) = sum transformationCounts
...
{-
    *** TODO ***
    ...
    Răspuns: permite calcularea valorilor doar atunci cand sunt necesare. Daca avem
             o pereche si dorim sa accesam doar o componenta a acesteia, evaluarea 
             lenesa asigura ca doar componenta accesata este calculata, iar cealalta 
             componenta ramane neevaluata. Acest lucru poate duce la economii 
             semnificative de timp si resurse, in special in cazul in care calcularea 
             componentei neaccesate ar fi costisitoare.
...
basicEntityCount = foldRegionAST combiner
  where
    -- 'combiner' este o functie care defineste cum sa numaram entitatile si transformarile.
    combiner :: RegionCombiner (Int, Int)
    -- Daca regiunea este definita de puncte, numaram ca o entitate.
    combiner (FromPoints _) = (1, 0)
    -- Daca regiunea este un dreptunghi, numaram ca o entitate.
    combiner (Rectangle _ _) = (1, 0)
    -- Daca regiunea este un cerc, numaram ca o entitate.
    combiner (Circle _) = (1, 0)
    -- Daca regiunea este complementul unei alte regiuni, numaram entitatile din regiunea originala.
    combiner (Complement region) = region
    -- Daca regiunea este uniunea a doua regiuni, numaram entitatile din ambele regiuni.
    combiner (Union r1 r2) = (fst r1 + fst r2, snd r1 + snd r2)
    -- Daca regiunea este intersectia a doua regiuni, numaram entitatile din ambele regiuni.
    combiner (Intersection r1 r2) = (fst r1 + fst r2, snd r1 + snd r2)
    -- Daca regiunea este transformarea unei alte regiuni, numaram entitatile din regiunea originala si adaugam numarul de transformari.
